...
## 02 lower case hashtags #
lower_tweets = tweets.map(lambda x: x.lower())
## 03 filter the words to get only hashtags, then map each hashtag to be a pair of (hashtag,1)
hashtags = lower_tweets.filter(lambda w: '#' in w).map(lambda x: (x, 1))
# 04 adding the count of each hashtag to its last count
